...
y = a2_true*x1 + a1_true*x + a0_true # <---- m_true*x+b_true
y_true = y
...
plot(x1, y,
     ylim=range(c(y-yerr, y+yerr)),
     pch=19, xlab="x2", ylab="y +/- yerr",
     main="Scatter plot with error bars"
)
# hack: we draw arrows but with very special "arrowheads"
arrows(x1, y-yerr, x1, y+yerr, length=0.05, angle=90, code=3)
lines(x1,y_true,col='red')
########################################################################
#yerr <- rep(1,N)
X <- matrix(c(rep(1.0,N),x,x1),nrow=N,ncol=3,byrow =FALSE)/matrix(c(yerr,yerr,yerr),nrow=N,ncol=3,byrow =FALSE)
...
rownames(our_fit_result) <- c("Intercept","Coeff. of x","Coeff. of x1")
colnames(our_fit_result) <- c("Estimate","Std. Error")
cat('Our fit results:----\n')
print(our_fit_result)
##############################################################################
##############################################################################
multilinear.model <-lm(y ~ x + x1,weights = 1/yerr^2)
summary(multilinear.model)
m <- summary(multilinear.model)
print(m)
print(m$sigma)
